module Spree
  module Api
    ...
    def express        
		...
		@order = Spree::Order.where( user_id: @user.id ).includes(:adjustments).order(created_at: :desc).take(1)
		order = @order[0]
		#order = Spree::Order.incomplete.includes(:adjustments).lock(false).find_by(current_order_params)
      ...
      begin
        ...
        if pp_response.success?
          #redirect_to provider.express_checkout_url(pp_response, useraction: 'commit')
                              render :json => {:status => 2 , :url => provider.express_checkout_url(pp_response, useraction: 'commit') }.to_json
		  			return
        else
          flash[:error] = Spree.t('flash.generic_error', scope: 'paypal', reasons: pp_response.errors.map(&:long_message).join(" "))
          #redirect_to checkout_state_path(:payment)
                              render :json => {:status => 5 , :error => "Order was created not right" }.to_json
		  			return
        end
      rescue SocketError
...
def confirm
      #order = current_order || raise(ActiveRecord::RecordNotFound)
      @user = Spree::User.find_by( spree_api_key: params[:spree_token] )
		@order = Spree::Order.where( user_id: @user.id ).includes(:adjustments).order(created_at: :desc).take(1)
		order = @order[0]
      ...
      if order.complete?
        ...
        redirect_to "http://watchr.42team.ru/buyer-step/3?action=complete"
      else
        #redirect_to checkout_state_path(order.state)
        redirect_to "http://watchr.42team.ru/buyer-step/3?action=fail"
      end
    end
    ...
    def express_checkout_request_details order, items
      { SetExpressCheckoutRequestDetails: {
          ...
          ReturnURL: "http://watchr.42team.ru/api/paypal/confirm/?spree_token=#{params[:token]}",
          CancelURL: "http://watchr.42team.ru/api/paypal/cancel/?spree_token=#{params[:token]}",
