...
namespace :spree_api_exchange do
  ...
  namespace :rates do
    ...
    task :ecb, [:load_currencies] => :environment do |t, args|
      ...
      phound = Spree::CurrencyConverter.find_by_currency_id(1)
      dollar = Spree::CurrencyConverter.find_by_currency_id(2)
      euro = Spree::CurrencyConverter.find_by_currency_id(3)
		

		Spree::Price.where(currency: "EUR").delete_all
		Spree::Price.where(currency: "USD").delete_all


		@variants =	Spree::Price.where(currency: "GBP")

			@variants.each do |i|
				i.amount = ((i.amount/phound.nominal) * dollar.nominal).round(2)
				i.currency = "USD"
				Spree::Price.create(variant_id: i.variant_id, amount: i.amount, currency: i.currency)
			end

			@variants.each do |i|
				i.amount = ((i.amount/phound.nominal) * euro.nominal).round(2)
				i.currency = "EUR"
				Spree::Price.create(variant_id: i.variant_id, amount: i.amount, currency: i.currency)
			end
